...
def preprocess_stop_stem(text, stem=False, stop=False, sent=False):
    # Remove punctuations
...
def preprocess_doc(docs, length = 2000, fix_length=True):
    new_docs = []
    for doc in docs:
        doc = clean_str((doc).encode('utf-8'))
        
        if fix_length:
            doc = preprocess_stop_stem(doc, stop=True, sent=True, stem=False)
            words = doc.split(' ')
        
            # Cut or Pad data
            if len(words) >= length: # Cut
                words = words[:length]
            else: # Pad at the end
                words = ["<pad>"] + words + ['<pad>'] * (length - len(words)-1) 
        
            # Convert word list back to a string string sentence
            new_doc = " ".join(words)
        else:
            new_doc = preprocess_stop_stem(doc, stop=True, sent=False, stem=False)
